    ...
    for k,v in raw_pickle.items():
        if k.startswith('['): #Skip token_usage
            ...
            for idx, qn_idx in enumerate(qns_idx):
                results[qn_idx] = extract_ans(v, f'{answer_prefix}{idx+1}:') #We start with question 1
    return results

def cal_metrics(EXP_NAME, test_df, classes, class_desp, show_error=True, bootstrap=1000):
    """
    ...
    results = convert_pkl(results) #Convert the batched results into individual answers
    ...
    for i in test_df.itertuples():
        ...
        if pred_idx >=0:
            ...
            if show_error:
                print(i.Index, f'multiple predictions found. raw response = {pred_text}')
            num_errors += 1
